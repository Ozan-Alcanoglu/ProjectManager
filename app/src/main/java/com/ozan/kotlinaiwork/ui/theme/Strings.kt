package com.ozan.kotlinaiwork.ui.theme

/**
 * Uygulama genelinde kullanılan string değerler
 */
object Strings {
    // Common
    const val APP_NAME = "Hobi Proje Yöneticisi"
    ...
    const val PRIORITY_LOW = "Düşük"
    const val PRIORITY_MEDIUM = "Orta"
    const val PRIORITY_HIGH = "Yüksek"
    const val PROJECT_STATUS = "Durum"
    ...
    const val EDIT_TASK = "Görevi Düzenle"
    const val TASK_TITLE = "Görev Başlığı"
    const val TASK_DESCRIPTION = "Açıklama (İsteğe Bağlı)"
    const val TASK_DUE_DATE = "Son Tarih"
    ...
    const val PROJECT_DELETED = "Proje silindi"
    const val TASK_ADDED = "Görev başarıyla eklendi"
    const val TASK_UPDATED = "Görev başarıyla güncellendi"
    const val TASK_DELETED = "Görev silindi"
}
